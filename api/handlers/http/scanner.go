...
func CreateScanner(svcGetter ServiceGetter[*service.ScannerService]) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if req.ScanType == "CISCO" {
			if err := validateCiscoScanner(req.Ip, req.Port, req.Username, req.Password, req.Protocol, true); err != nil {
				context.GetLogger(c.UserContext()).Error("Cisco scanner validation failed", "error", err)
				return c.Status(fiber.StatusBadRequest).JSON(ErrorResponse{
					Success: false,
					Error:   err.Error(),
				})
...
func UpdateScanner(svcGetter ServiceGetter[*service.ScannerService]) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if req.ScanType == "CISCO" {
			if err := validateCiscoScanner(req.Ip, req.Port, req.Username, req.Password, req.Protocol, false); err != nil {
				context.GetLogger(c.UserContext()).Error("Cisco scanner update validation failed", "error", err)
				return c.Status(fiber.StatusBadRequest).JSON(ErrorResponse{
					Success: false,
					Error:   err.Error(),
				})
...
// validateCiscoScanner validates Cisco-specific fields
func validateCiscoScanner(ip, port, username, password, protocol string, isCreateOperation bool) error {
	// For create operations, all basic fields are required
	if isCreateOperation {
		if ip == "" {
			return fmt.Errorf("IP address is required for Cisco scanner")
		}
		if port == "" {
			return fmt.Errorf("port is required for Cisco scanner")
		}
		if username == "" {
			return fmt.Errorf("username is required for Cisco scanner")
		}
		if password == "" {
			return fmt.Errorf("password is required for Cisco scanner")
		}
		if protocol == "" {
			return fmt.Errorf("protocol is required for Cisco scanner")
		}
	}

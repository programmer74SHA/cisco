...
func (s *scannerService) validateScanner(scanner *domain.ScannerDomain) error {
	...
	// Note: Firewall validation is handled in HTTP layer
	...
	case domain.ScannerTypeFirewall:
		// Firewall validation is handled in the HTTP layer for more comprehensive validation
		// including IP format validation and different logic for create vs update operations
		log.Printf("Service: Firewall scanner validation handled in HTTP layer")

	case domain.ScannerTypeCisco:
		if err := s.validateCiscoScanner(scanner); err != nil {
			return err
		}
...
func (s *scannerService) CreateScanner(ctx context.Context, scanner domain.ScannerDomain) (int64, error) {
	...
	// Ensure Nmap profile is properly set for Nmap scanners
...
func (s *scannerService) ensureNmapProfileLoaded(ctx context.Context, scanner *domain.ScannerDomain) error {
	...
	switch scanner.Type {
	case "custom":
		// For custom switches, don't load any profile
		scanner.NmapProfile = nil
		scanner.NmapProfileID = nil
		log.Printf("Service: Nmap scanner using custom switches, no profile loaded")
		return nil
...
func (s *scannerService) validateCiscoScanner(scanner *domain.ScannerDomain) error {
	// Validate required fields
	if scanner.IP == "" {
		return fmt.Errorf("Cisco scanner requires IP address")
	}
	if scanner.Username == "" {
		return fmt.Errorf("Cisco scanner requires username")
	}
	if scanner.Password == "" {
		return fmt.Errorf("Cisco scanner requires password")
	}

...
func (r *assetRepository) getAssetIPs(ctx context.Context, assetID string) ([]types.AssetIPs, error) {
	logger.DebugContext(ctx, "Repository: Getting asset IPs for asset ID: %s", assetID)

	var assetIPs []types.AssetIPs
...
func (r *assetRepository) GetByFilter(ctx context.Context, assetFilter domain.AssetFilters, limit, offset int, sortOptions ...domain.SortOption) ([]domain.AssetDomain, int, error) {
	...
	if hasGroupBy && len(assets) > 0 {
		...
		ipMap := make(map[string][]types.AssetIPs)
...
func (r *assetRepository) Update(ctx context.Context, asset domain.AssetDomain) error {
	...
	currentIPMap := make(map[string]types.AssetIPs)
	...
	var ipsToDelete []types.AssetIPs
	...
	if ipsChanged {
		...
		if len(ipsToAdd) > 0 {
			...
			var conflictActiveIPs []types.AssetIPs
			...
			var newIPsToCreate []*types.AssetIPs
...
func (r *assetRepository) updateOrUndeleteIP(tx *gorm.DB, foundIP types.AssetIPs, newAssetID string, macAddress string) error {
...
func (r *assetRepository) createNewIPs(tx *gorm.DB, assetIPs []*types.AssetIPs, processedIPs map[string]bool) error {
...
func (r *assetRepository) findExistingIPs(ctx context.Context, ipAddresses []string) ([]types.AssetIPs, []types.AssetIPs, error) {
	logger.DebugContext(ctx, "Repository: Finding existing IPs for %d addresses", len(ipAddresses))

	var foundIPs []types.AssetIPs
	...
	var existingActiveIPs []types.AssetIPs
	var existingDeletedIPs []types.AssetIPs
...
func (r *assetRepository) checkActiveIPsAssets(ctx context.Context, activeIPs []types.AssetIPs) (bool, error) {
	if len(activeIPs) == 0 {
...
func (r *assetRepository) handleExistingIPs(ctx context.Context, tx *gorm.DB, asset domain.AssetDomain,
	validAssetIPs []domain.AssetIP, assetRecord *types.Assets, assetIPs []*types.AssetIPs, portRecords []types.Port) error {
...
func (r *assetRepository) checkActiveIPsAssetsWithUpdate(ctx context.Context, activeIPs []types.AssetIPs, newScannerType string) (bool, string, error) {
	if len(activeIPs) == 0 {
		logger.DebugContext(ctx, "Repository: No active IPs to check")
		return false, "", nil
	}

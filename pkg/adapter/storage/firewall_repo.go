...
func (r *FirewallRepo) storeZones(tx *gorm.DB, zones []domain.ZoneData) (map[string]string, error) {
...
func (r *FirewallRepo) StoreFirewallDataWithAssets(ctx context.Context, data domain.FirewallData, scanJobID int64, interfaceAssetMap map[string]string) error {
	...
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		zoneMap, err := r.storeZones(tx, data.Zones)
		if err != nil {
		...
		vlanIDMap, err := r.storeVLANs(tx, data.VLANs, interfaceMap)
		if err != nil {
		...
		if err := r.storeZoneDetails(tx, data.Interfaces, zoneMap, interfaceMap, vlanIDMap); err != nil {
		...
		// Step 7: Get firewall details ID for policy storage

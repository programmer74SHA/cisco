...
type app struct {
	...
	ciscoScanner      *scanner.CiscoRunner
...
func NewApp(cfg config.Config) (AppContainer, error) {
	...
	a.ciscoScanner = scanner.NewCiscoRunner(assetRepo, a.db)
	...
	if a.ciscoScanner == nil {
		coreLogger.Warn("CiscoScanner was not initialized properly")
	} else {
		coreLogger.Info("CiscoScanner initialized successfully")
	}
	...
	a.scannerFactory.RegisterScanner("CISCO", a.ciscoScanner)
...
func (a *app) schedulerServiceWithDB(db *gorm.DB) schedulerPort.Service {
	...
	ciscoScanner := scanner.NewCiscoRunner(assetRepo, db)
	...
	scannerFactory.RegisterScanner("CISCO", ciscoScanner)
